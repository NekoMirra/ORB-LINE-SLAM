...
public:
    ...
    // Fast pose optimization for reprojection error monitoring
    // 快速位姿优化，仅优化关键帧位姿(SE3)，用于重投影误差监控
    void static FastPoseOptimization(const std::vector<KeyFrame*>& vpKeyFrames, 
                                    const std::vector<MapPoint*>& vpMapPoints,
                                    const std::vector<MapLine*>& vpMapLines = std::vector<MapLine*>(),
                                    int nIterations = 3);

    // Enhanced fast pose optimization with result validation and safe update
    // 增强的快速位姿优化，包含结果验证和安全更新
    struct OptimizationResult {
        ...
        double duration;
        int iterations;
        std::vector<cv::Mat> poseChanges;
        std::vector<KeyFrame*> optimizedKFs;
    };
    
    static OptimizationResult FastPoseOptimizationWithValidation(
    ...
    // Thread-safe pose update with connection management
    // 线程安全的位姿更新，包含连接关系管理
    static void SafeUpdateKeyFramePoses(const std::vector<KeyFrame*>& vpKeyFrames,
                                       const std::vector<cv::Mat>& newPoses,
                                       bool updateConnections = true);

    // Helper methods for enhanced optimization
    // 增强优化的辅助方法
    static double ComputeReprojectionError(const std::vector<KeyFrame*>& vpKeyFrames,
                                         const std::vector<MapPoint*>& vpMapPoints);
    
    static void UpdateMapPointVisibility(KeyFrame* pKF);
    
    static void UpdateKeyFrameConnections(const std::vector<KeyFrame*>& vpKeyFrames);
};
