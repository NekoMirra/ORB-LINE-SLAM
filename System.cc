...
// Reprojection error monitoring methods implementation
// 重投影误差监控方法实现

bool System::CheckReprojectionError()
{
    ...
    // 获取当前帧
    Frame* pCurrentFrame = mpTracker->GetCurrentFrame();
    if (!pCurrentFrame) {
        return false;
    }
    
    // 计算重投影误差
    double reprojectionError = mpReprojectionErrorMonitor->ComputeReprojectionError(pCurrentFrame);
    
    // 更新监控器并检查是否需要触发优化
    bool shouldOptimize = mpReprojectionErrorMonitor->UpdateReprojectionError(pCurrentFrame, reprojectionError);
    
    if (shouldOptimize) {
        // 触发快速位姿优化
        TriggerFastPoseOptimization();
    }
    
    return shouldOptimize;
}

ReprojectionErrorMonitor::ErrorStats System::GetReprojectionErrorStats()
{
    if (mpReprojectionErrorMonitor) {
        return mpReprojectionErrorMonitor->GetErrorStats();
    }
    
    // 返回默认值
    ReprojectionErrorMonitor::ErrorStats stats;
    stats.currentError = 0.0;
    stats.averageError = 0.0;
    stats.changeRate = 0.0;
    stats.maxError = 0.0;
    stats.minError = 0.0;
    stats.errorCount = 0;
    stats.lastUpdateTime = 0.0;
    
    return stats;
}

void System::SetReprojectionErrorThresholds(double errorThreshold, double changeRateThreshold, double durationThreshold)
{
    if (mpReprojectionErrorMonitor) {
        mpReprojectionErrorMonitor->SetParameters(20, errorThreshold, changeRateThreshold, durationThreshold);
    }
}

void System::TriggerFastPoseOptimization()
{
    if (!mpReprojectionErrorMonitor || !mpAtlas) {
        return;
    }
    ...
    // 获取当前地图中的关键帧和地图点
    vector<KeyFrame*> vpKeyFrames = pCurrentMap->GetAllKeyFrames();
    vector<MapPoint*> vpMapPoints = pCurrentMap->GetAllMapPoints();
    vector<MapLine*> vpMapLines = pCurrentMap->GetAllMapLines();
    ...
    // 记录优化结果
    ReprojectionErrorMonitor::OptimizationRecord record;
    record.timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(
        startTime.time_since_epoch()).count() / 1000.0;
    record.preError = result.initialError;
    record.postError = result.finalError;
...
void System::UpdateKeyFramePoses(const vector<KeyFrame*>& vpKeyFrames)
{
    for (KeyFrame* pKF : vpKeyFrames) {
        if (pKF && !pKF->isBad()) {
            // 更新关键帧的位姿
            pKF->UpdatePoseFromOptimization();
        }
    }
}
