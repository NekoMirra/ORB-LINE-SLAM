...
void Optimizer::FastPoseOptimization(const std::vector<KeyFrame*>& vpKeyFrames, 
                                    const std::vector<MapPoint*>& vpMapPoints,
                                    const std::vector<MapLine*>& vpMapLines,
                                    int nIterations)
{
    ...
    for(size_t i=0; i<vpMapLines.size(); i++)
    {
        ...
        // 为线的起点和终点分别创建顶点
        g2o::VertexPointXYZ* vPointStart = new g2o::VertexPointXYZ();
        vPointStart->setEstimate(Converter::toVector3d(pML->GetWorldPosStart()));
        const int idStart = pML->mnId * 2 + maxKFid + 1 + vpMapPoints.size();
        vPointStart->setId(idStart);
        vPointStart->setFixed(true);
        optimizer.addVertex(vPointStart);

        g2o::VertexPointXYZ* vPointEnd = new g2o::VertexPointXYZ();
        vPointEnd->setEstimate(Converter::toVector3d(pML->GetWorldPosEnd()));
        const int idEnd = pML->mnId * 2 + 1 + maxKFid + 1 + vpMapPoints.size();
        vPointEnd->setId(idEnd);
        vPointEnd->setFixed(true);
        optimizer.addVertex(vPointEnd);
    }
    ...
    for(size_t i=0; i<vpKeyFrames.size(); i++)
    {
        ...
        const cv::Mat Rcw = pKF->GetRotation();
        const cv::Mat tcw = pKF->GetTranslation();
        const cv::Mat Ow = pKF->GetCameraCenter();

        for(int j=0; j<pKF->N; j++)
        {
            MapPoint* pMP = pKF->mvpMapPoints[j];
            if(!pMP)
                continue;
            if(pMP->isBad())
                continue;
            ...
            const float u = pKF->mvKeysUn[j].pt.x;
            const float v = pKF->mvKeysUn[j].pt.y;

            const float chi2 = pKF->mvLevelSigma2[j];
            ...
            e->setInformation(Eigen::Matrix2d::Identity() * 1.0f/chi2);
            g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
            e->setRobustKernel(rk);
            rk->setDelta(sqrt(chi2));
            optimizer.addEdge(e);
        }
    }
    ...
    for(size_t i=0; i<vpKeyFrames.size(); i++)
    {
        ...
        for(int j=0; j<pKF->Nlines; j++)
        {
            MapLine* pML = pKF->mvpMapLines[j];
            if(!pML)
                continue;
            if(pML->isBad())
                continue;
    ...
    cout << "Fast pose optimization completed with " << nIterations << " iterations" << endl;
}

Optimizer::OptimizationResult Optimizer::FastPoseOptimizationWithValidation(
    const std::vector<KeyFrame*>& vpKeyFrames,
    const std::vector<MapPoint*>& vpMapPoints,
    const std::vector<MapLine*>& vpMapLines,
    int nIterations,
    double convergenceThreshold)
{
    ...
    result.initialError = 0.0;
    result.finalError = 0.0;
    result.improvement = 0.0;
    result.duration = 0.0;
    result.iterations = 0;
    result.optimizedKFs = vpKeyFrames;
    ...
    // 保存初始位姿
    std::vector<cv::Mat> initialPoses;
    for(const auto& pKF : vpKeyFrames) {
        if(!pKF->isBad()) {
            initialPoses.push_back(pKF->GetPose().clone());
        }
    }
    
    // 计算初始重投影误差
    result.initialError = ComputeReprojectionError(vpKeyFrames, vpMapPoints);
    ...
    for(size_t i=0; i<vpKeyFrames.size(); i++)
    {
        ...
        for(int j=0; j<pKF->N; j++)
        {
            ...
            const cv::KeyPoint &kpUn = pKF->mvKeysUn[j];
            const float &kpx = kpUn.pt.x;
            const float &kpy = kpUn.pt.y;
            const float invSigma2 = pKF->mvInvLevelSigma2[kpUn.octave];

            g2o::EdgeSE3ProjectXYZ* e = new g2o::EdgeSE3ProjectXYZ();
            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pMP->mnId + maxKFid + 1)));
            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mnId)));
            e->setMeasurement(Eigen::Vector2d(kpx, kpy));
            e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
    ...
    if(edgeCount < 10) {
        std::cout << "Warning: Too few edges for optimization: " << edgeCount << std::endl;
        return result;
    }
    ...
    double lastError = optimizer.activeChi2();
    result.iterations = 0;
    
    for(int iter = 0; iter < nIterations; iter++) {
        optimizer.optimize(1);
        double currentError = optimizer.activeChi2();
        
        result.iterations++;
        
        // 检查收敛
        if(iter > 0 && std::abs(lastError - currentError) < convergenceThreshold) {
    ...
    for(size_t i=0; i<vpKeyFrames.size(); i++)
    {
        ...
        // 计算位姿变化
        cv::Mat poseChange = pose * pKF->GetPoseInverse();
        result.poseChanges.push_back(poseChange);
    }

    // 计算最终误差
    result.finalError = ComputeReprojectionError(vpKeyFrames, vpMapPoints);
    result.improvement = (result.initialError - result.finalError) / result.initialError;
    
    auto endTime = std::chrono::steady_clock::now();
    result.duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime).count();
    ...
    // 如果结果有效，应用位姿更新
    if (validResult && result.improvement > 0.01) {
        SafeUpdateKeyFramePoses(vpKeyFrames, newPoses, true);
        std::cout << "Optimization successful. Error improved from " << result.initialError 
                  << " to " << result.finalError << " (" << result.improvement * 100 << "%)" << std::endl;
    } else {
        // 恢复初始位姿
        for(size_t i = 0; i < vpKeyFrames.size() && i < initialPoses.size(); i++) {
            if(!vpKeyFrames[i]->isBad()) {
                vpKeyFrames[i]->SetPose(initialPoses[i]);
            }
        }
        std::cout << "Optimization failed or insufficient improvement. Poses restored." << std::endl;
    }

    return result;
}

void Optimizer::SafeUpdateKeyFramePoses(const std::vector<KeyFrame*>& vpKeyFrames,
                                       const std::vector<cv::Mat>& newPoses,
                                       bool updateConnections)
{
    if(vpKeyFrames.size() != newPoses.size()) {
        std::cerr << "Error: Size mismatch in SafeUpdateKeyFramePoses" << std::endl;
        return;
    }

    // 按ID排序关键帧以避免死锁
    std::vector<std::pair<KeyFrame*, cv::Mat>> sortedUpdates;
    for(size_t i = 0; i < vpKeyFrames.size(); i++) {
        if(!vpKeyFrames[i]->isBad()) {
            sortedUpdates.push_back(std::make_pair(vpKeyFrames[i], newPoses[i]));
        }
    }
    
    std::sort(sortedUpdates.begin(), sortedUpdates.end(), 
              [](const std::pair<KeyFrame*, cv::Mat>& a, const std::pair<KeyFrame*, cv::Mat>& b) {
                  return a.first->mnId < b.first->mnId;
              });

    // 按顺序更新位姿
    for(const auto& update : sortedUpdates) {
        KeyFrame* pKF = update.first;
        const cv::Mat& newPose = update.second;
        
        // 获取关键帧的互斥锁
        std::unique_lock<std::mutex> lock(pKF->mMutexPose);
        
        // 更新位姿
        pKF->SetPose(newPose);
        
        // 更新地图点的可见性
        UpdateMapPointVisibility(pKF);
    }

    // 更新关键帧连接关系
    if(updateConnections) {
        UpdateKeyFrameConnections(vpKeyFrames);
    }
}

double Optimizer::ComputeReprojectionError(const std::vector<KeyFrame*>& vpKeyFrames,
                                         const std::vector<MapPoint*>& vpMapPoints)
{
    ...
    for(const auto& pKF : vpKeyFrames) {
        ...
        for(int i = 0; i < pKF->N; i++) {
            ...
            cv::Mat p3D_cam = pKF->GetRotation() * p3D + pKF->GetTranslation();

            if(p3D_cam.at<float>(2) <= 0) continue;

            float x = p3D_cam.at<float>(0) / p3D_cam.at<float>(2);
            float y = p3D_cam.at<float>(1) / p3D_cam.at<float>(2);

            cv::Point2f observed = pKF->mvKeysUn[i].pt;
    ...
    return validObservations > 0 ? totalError / validObservations : 0.0;
}

void Optimizer::UpdateMapPointVisibility(KeyFrame* pKF)
{
    // 更新地图点在该关键帧中的可见性
    for(int i = 0; i < pKF->N; i++) {
        MapPoint* pMP = pKF->mvpMapPoints[i];
        ...
        if(p3D_cam.at<float>(2) <= 0) {
            // 地图点在相机后方，标记为不可见
            pKF->mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
            continue;
        }
...
void Optimizer::UpdateKeyFrameConnections(const std::vector<KeyFrame*>& vpKeyFrames)
{
    // 更新关键帧之间的连接关系
